...
macro_rules! from {
    ...
    ($(@$t: ident => $p: expr)*) => (
        $(
        impl <'g> From<&'g YouTrack> for $t<'g> {
            fn from(yt: &'g YouTrack) -> Self {
                ...
                let res = Request::builder().method($p)
                    ...
                    .map_err(From::from)
                    .and_then(|req| {
                        let token = String::from("Bearer ") + &yt.token;
                    ...
                    Ok((mut req, token)) => {
                        {
                            ...
                            headers.insert(USER_AGENT, HeaderValue::from_static("YouTrack-rs"));
